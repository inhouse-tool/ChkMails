...
1. 「失敗」と判定されなかった場合:<br>「なんか受信した」とアプリ層に通知.
...
冒頭で紹介した `ほぼ存在しない` は, 何を学習したのか,
その後は
```
結論から言うと、Windows の secur32.dll を使って TLS 1.3 を有効化・利用できたという成功報告は存在しません。
理由は技術仕様上はっきりしていて、TLS 1.3 の実装は Schannel（schannel.dll）側にあり、
secur32.dll では TLS 1.3 を提供していないためです。
```
と, 屁理屈をこねるようになっていました. <sub>( 2026年6月現在 )</sub>

&nbsp;  
&nbsp;  

このままだとなんか後味が悪いので, [動作例](TLS1.3.Example.md) もご参考にどうぞ.
