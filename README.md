# ChkMails<br><sup><sup>Version 1.1.16.920</sup></sup>
...
届いたメールの[メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)を見ると, 認証結果が判ります.
...
この「名義」とは,
[メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)の `From:` 欄に書いてある「名義」のことで,
`<` と `>` で囲まれた送信者のメールアドレスと,
...
あと, Alias 詐称業界の新しいトレンドとして,
「どこのフリもしない」
という手法も現れました.
有名企業を騙ったりするのではなく,
`サポート係`
とか, 何の固有名詞も含まない抽象的な名乗りに留めておくという手法です.
どこも騙っていないので,
ドメインの照合とかしても無意味です.

メールの見た目がアヤしくなるので, 人が目で見たら即破棄されるでしょうが,
受信メールボックスに到達しやすくはなるので, ウザいでしょう.
...
などと登録しておくことです.
ドメインに `!` を含むメールアドレスなんて存在しないので,
この登録内容だと

「alias に `サポート係` を含むメールは破棄する」

ということになります.

他に `<企業名>サポート係` を名乗ってくる取引先があって巻き添えになってしまう場合は,
`"サポート係"` とダブルクオートで囲んだ Alias を登録しましょう.
...
こちらの場合,
登録した Alias を「含む」メールを全て破棄するのではなく,
登録した Alias に「一致する」メールだけを破棄します.
さすがに `サポート係` とだけ名乗ってくるメールは,
マトモな企業からは来ないでしょう.
...
と,
知らない人のメールアドレスが[メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)に書いてあったりします.
そして, `To:` や `Cc:` のどこにも
...
1. 標準にないヘッダーフィールドを含むメール<sup>※</sup><br>
[メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)には, `From:`, `To:`, `Date:`, `Subject:` 
等いろいろな[ヘッダーフィールド](https://atmarkit.itmedia.co.jp/fnetwork/rensai/netpro03/mail-header.html)があります.
規則上はこれらを綴る際に大文字小文字は気にしないことになっていますが,
マトモなメール環境から発せられたメールは,
それぞれのヘッダーフィールドの意味を定義した
[RFC](https://ja.wikipedia.org/wiki/Request_for_Comments)
が定めた通りの綴り方 ( 大文字小文字の使い分け方 ) に従っています.<br>
スパムメールにはこの辺がいいかげんなものが多く,
さらにはどの RFC にも定められていないヘッダーフィールドを勝手に使っているものもあったりするので,
判断の材料の一つとして加えてあります.<br>
<sup>
※ 中国と交流のある方はこの☑を外してください.
かなりの高確率で先方からのメールが破棄されてしまいます.
</sup>
...
デフォルトで, 1. から 6. までは ON, 7. は OFF にしています.
...
[メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)にある `X-mailer:` 欄での自己申告をアテに,
...
送信者の正確な名義
( [メールヘッダー](https://www.dekyo.or.jp/soudan/contents/ihan/header.html)の `From:` にあるメールアドレス ) と,
...
| `Sender Setting Unconventional Header`| [Sender](#sender) の「標準にないヘッダーフィールドを含むメール」を検出	|
